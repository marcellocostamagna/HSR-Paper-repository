...
molecule = load_molecules_from_sdf(f'{cwd}/molecule_2.sdf', removeHs=False, sanitize=False)[0]
...
# Rhenium coordinates
for atom in molecule.GetAtoms():
    if atom.GetSymbol() == 'Re':
        position = molecule.GetConformer().GetAtomPosition(atom.GetIdx())
        Re_coords = [position.x, position.y, position.z]
...
# Load the molecule
molecule_path = f'{cwd}/molecule_2.sdf'
...
cmd.pseudoatom('Re', pos=Re_coords, color='yellow')
...
# Example usage
dotted_line_obj = create_dotted_line(Re_coords, penta_ctd, color=[0.5, 0.5, 0.5], segments=20)
