...
molecule = load_molecules_from_sdf(f'{cwd}/molecule_5.sdf', removeHs=False, sanitize=False)[0]
...
# ethylene carbons: 16, 19
# Get coordinates of the center of the ethylene(propylene) ligand
ethylene_coords = []
for atom in molecule.GetAtoms():
    if atom.GetSymbol() == 'C':
        if atom.GetIdx() in [15, 18]:
            position = molecule.GetConformer().GetAtomPosition(atom.GetIdx())
            ethylene_coords.append([position.x, position.y, position.z])
ethylene_coords = np.array(ethylene_coords)
ethylene_ctd = (np.mean(ethylene_coords, axis=0)).tolist()
...
# Load the molecule
molecule_path = f'{cwd}/molecule_5.sdf'
...
dotted_line_obj_1 = create_dotted_line(Mn_coords, ethylene_ctd, color=[0.5, 0.5, 0.5], segments=20)
cmd.load_cgo(dotted_line_obj_1, 'dotted_line_2')
